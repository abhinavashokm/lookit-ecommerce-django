...
def cart(request):
    ...
    if cart_applied_exist:
        ...
        if applied_coupon.discount_type == 'FLAT':
            ...
            cart_summary['coupon_discount'] = (cart_summary['grand_total'] * applied_coupon.discount_value) / 100 
        cart_summary['grand_total'] -= cart_summary['coupon_discount']

    return render(
        request,
        'cart/cart.html',
        {"cart_items": cart_items, "cart_summary": cart_summary, "checkout_block":checkout_block, "saved_coupons":saved_coupons, "applied_coupon":applied_coupon},
    )
...
def apply_coupon(request):
    if request.method == "POST":
        ...
        applied_coupon_exist = CartAppliedCoupon.objects.filter(user=request.user).exists()
