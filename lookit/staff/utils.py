...
from django.db.models import Count, Sum, DecimalField, F, ExpressionWrapper, IntegerField
...
def get_top_selling_styles():
    ...
    top_selling_styles = (
        ...
        .annotate(
            sale_count = Count('product__variant__orders')
        )
        .annotate(
    ...
    base_for_calculation = top_selling_styles[0].sale_count or 1  # prevent divide by zero

    # annotate percentage for each style (for graph representation)
    top_selling_styles = top_selling_styles.annotate(
        sale_percentage=ExpressionWrapper(F('sale_count') * 100 / base_for_calculation, output_field=IntegerField())
    )
